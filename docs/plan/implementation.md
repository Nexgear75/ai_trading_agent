...
> Ce plan découpe l'implémentation en **Work Streams (WS)** — partiellement parallélisables au sein d'un même milestone (ex. WS-3.1 à 3.5, WS-9.1/9.2/9.3, WS-10.1/10.2), mais chaînés séquentiellement entre milestones — et en **Milestones (M)** séquentiels.
...
flowchart LR
  ...
  subgraph M3["Milestone 3 — Training Framework + Backtest Engine"]
    WS6[WS-6 Interface modèle]
    WS7[WS-7 Calibration θ]
    WS8a[WS-8.1/8.2/8.3 Backtest engine]
    GD{G-Doc}
  end
  subgraph M4["Milestone 4 — Baselines, Métriques & Journal"]
    WS8b[WS-8.4 Trade journal]
    GB{G-Backtest}
    WS9[WS-9 Baselines]
...
- **Pré-gate (G-Doc)** : vérification structurelle positionnée après tous les WS de M3, servant de pré-condition au gate M3. Bloquant pour le gate M3.
...
|---|---|---|---|---|
| **M1** | Configuration et données sources prêtes | `0` erreur de validation config, `0` erreur QA bloquante, `>= 95%` de couverture de tests sur les modules de configuration et données (WS-1.2, WS-1.3, WS-2) (mesurée via `pytest --cov=ai_trading.config --cov=ai_trading.data.ingestion --cov=ai_trading.data.qa --cov=ai_trading.data.missing --cov-fail-under=95`) | Log de chargement config, rapport QA, `pytest` ciblé WS-1.2/WS-1.3/WS-2 avec couverture, hash SHA-256 des données, rapport `gate_report_M1.json` | `GO` si les 3 seuils sont atteints, sinon `NO-GO` |
| **M2** | Pipeline data/feature causale et sans fuite | `0` fuite détectée par tests split/embargo, `100%` de disjonction train/val/test, reproductibilité `X_seq` bit-à-bit identique (même seed, même plateforme ; tolérance `atol=1e-7` pour cross-plateforme float32), taux de NaN hors warmup `= 0%`, anti-fuite scaler : indices `X_train` du `scaler.fit()` strictement disjoints de `X_val ∪ X_test` | Rapport de validation split/embargo, test anti-leakage, snapshot config, test de reproductibilité features | `GO` si les 5 seuils sont atteints |
...
| **M6** | Validation full-scale réseau | (a) `make run-all CONFIG=configs/fullscale_btc.yaml` s'exécute sans crash (exit code 0). (b) Données BTCUSDT 1h téléchargées depuis Binance couvrant `[2017-08-17, 2026-01-01[` (~73 000 bougies). (c) QA passe sur données réelles (0 erreur bloquante). (d) `manifest.json` et `metrics.json` valides (JSON Schema). (e) Au moins 1 fold complété avec métriques non nulles. (f) Artefacts complets (`equity_curve_stitched.csv`, `trades.csv` par fold, `config_snapshot.yaml`). (g) Tests fullscale (WS-13.2/13.3) : pas de fixture de données OHLCV ni de mock réseau — accès réseau réel obligatoire. Les tests de config (WS-13.1) peuvent utiliser des fixtures conftest standards pour la comparaison YAML. | Run directory complet, `manifest.json`, `metrics.json`, equity curves, trade journals, logs pipeline, rapport `gate_report_M6.json` | `GO` si (a)–(g) atteints, sinon `NO-GO` |
...
| **Labels** (WS-4.1) | `y_t` n'utilise que des prix dans `[t, t+H]` (`Open[t+1]` et `Close[t+H]` pour `log_return_trade`, `Close[t]` et `Close[t+H]` pour `log_return_close_to_close`) — pas de prix pour `t' > t+H` | Test unitaire : masquer les prix `t > t+H` → `y_t` identique |
...
| **Scaling** (WS-5) | `scaler.fit()` n'a jamais vu d'indices val/test | Assertion : `set(indices_fit) ⊂ set(indices_train)` et `set(indices_fit) ∩ set(indices_val ∪ indices_test) == ∅`. Inclus dans gate M2 (déplacé depuis G-Split car le scaler n'est pas encore disponible au moment de G-Split). |
...
| **Backtest** (WS-8) | Signaux à t ne dépendent pas des prix `t' > t` | Test de perturbation généralisé (cf. WS-12.2 test de causalité). Inclus dans G-Backtest critère (6). |
...
graph TD
  ...
  T34 --> T36
  T35 --> T36
  T36 --> T37[WS-3.7 Warmup + masque validité]
  T23 --> T37
  T37 --> GF{G-Features}
  ...
  T102 --> T103
  T12 -.-> T111[WS-11.1 Arborescence run]
  T111 --> T112[WS-11.2 Manifest builder]
  T45 --> T112
  T21 --> T112
  T103 --> T113[WS-11.3 Metrics builder]
  T72 --> T113
  T112 --> T114[WS-11.4 Schema validation]
  T113 --> T114
  T12 -.-> T121[WS-12.1 Seed manager]
...
| **Dépendances** | WS-1.2 (config pour symbole, timeframe, période), WS-1.3 (config validée avant utilisation) |
...
| Champ | Valeur |
|---|---|
| **Description** | Implémenter la politique MVP : pas d'interpolation. (1) Détecter les positions des bougies manquantes. (2) Marquer comme invalides tous les samples dont la fenêtre d'entrée `[t-L+1, t]` ou la fenêtre de sortie `[t+1, t+H]` touche un trou. (3) Retourner un masque de validité `valid_mask` de shape `(T,)` (`T` = nombre total de timestamps bruts ; `N` est réservé au nombre de samples post-filtrage, cf. WS-4). |
| **Réf. spec** | §4.3, §6.6 |
...
| **Réf. spec** | §6.2 (tableau canonique — pas de section dédiée volume dans la spec ; formules définies uniquement dans le tableau §6.2, comme les log-returns WS-3.1) |
...
2. **Chaque module feature** (`log_returns.py`, `volatility.py`, etc.) :
   ...
   - **Note (volatility.py)** : les features `vol_24` et `vol_72` recalculent `log(C_t / C_{t-1})` en interne à partir des clôtures brutes, plutôt que de dépendre de la sortie de la feature `logret_1`. Ce choix est intentionnel : il garantit l'indépendance de chaque feature dans le registre. Un helper interne `_compute_logret_1(close: pd.Series) -> pd.Series` peut optionnellement être extrait dans `features/_helpers.py` pour éviter la duplication de code (non livrable obligatoire — le fichier n'est créé que si le refactoring est jugé nécessaire).
...
3. **`features/__init__.py`** — Auto-registration :
   ...
   - Le même pattern s'applique à `models/__init__.py` pour le `MODEL_REGISTRY` (auto-import de `dummy.py` dès M3 ; les imports des baselines `no_trade.py`, `buy_hold.py`, `sma_rule.py` depuis le package `baselines/` ne sont ajoutés qu'à M4 quand ces modules existent, évitant un `ImportError` à M3).
...
### WS-3.7 — Warmup et masque de validité final

| Champ | Valeur |
|---|---|
| **Description** | (1) Appliquer `min_warmup` : invalider les `min_warmup` premières bougies. (2) Vérifier qu'aucune feature à t ne contient de NaN interne (hors zone warmup). (3) Combiner avec le `valid_mask` de WS-2.3 pour produire le masque final. **Note** : la validation de causalité proprement dite (test de perturbation : modifier les prix futurs → features identiques) est vérifiée au gate G-Features, pas dans cette tâche. |
| **Réf. spec** | §6.6 |
...
| **Automatisation** | `make gate-features` → `pytest tests/test_log_returns.py tests/test_volatility.py tests/test_rsi.py tests/test_ema_ratio.py tests/test_volume_features.py tests/test_feature_registry.py tests/test_feature_pipeline.py tests/test_warmup_validation.py -v --cov=ai_trading.features --cov-fail-under=90` + test de causalité + génération `reports/gate_report_G_Features.json` |
...
| **Dépendances** | G-Features (après WS-3.7) |
...
| **Dépendances** | WS-4.4 (métadonnées pour les timestamps des samples, inclut transitivement WS-4.2) |
...
**Exemple fold k=1 (illustration du micro-gap)** :

Avec `step_days=30` → le train du fold k=1 avance de 30 jours :

| Borne | Calcul | Valeur |
|---|---|---|
| `train_start[1]` | `train_start[0] + step_days` | `2024-01-31 00:00` |
| `train_val_end[1]` | `+ 180j - 1h` | `2024-07-28 23:00` |
| `test_start[1]` | `+ 180j + 4×1h` | `2024-07-29 04:00` |
| `test_end[1]` | `test_start + 30j - 1h` | `2024-08-28 03:00` |

On confirme : `test_end[0] = 2024-07-29 03:00` et `test_start[1] = 2024-07-29 04:00` → micro-gap de exactement `Δ = 1h`.
...
| **Critères** | (1) `100%` disjonction temporelle train/val/test par fold (aucun timestamp partagé). (2) Purge E2E (G-Leak/Splits) : `max(t + H × Δ for t in train_val_samples) < test_start` pour chaque fold. (3) `>= 1` fold valide après troncation et filtrage. (4) Tenseur `X_seq` reproductible bit-à-bit (même seed, même plateforme ; tolérance `atol=1e-7` cross-plateforme). (5) `0%` NaN dans `X_seq` et `y`. **Note** : la vérification anti-fuite scaler (G-Leak/Scaling : `set(indices_train) ∩ set(indices_val ∪ indices_test) == ∅`) est vérifiée au gate M2 (après WS-5.1), pas à G-Split (le scaler n'est pas encore disponible). |
| **Automatisation** | `make gate-split` → `pytest tests/test_dataset.py tests/test_splitter.py -v --cov=ai_trading.data.dataset --cov=ai_trading.data.splitter --cov-fail-under=90` + test E2E split/embargo + test reproductibilité tenseur + génération `reports/gate_report_G_Split.json` |
| **Décision** | `GO` si les 5 critères sont atteints. Bloquant pour WS-5. |
...
| **Dépendances** | G-Split (après WS-4.6) |
...
> **Note sur le parallélisme au sein de M3** : la numérotation WS-6 → WS-7 → WS-8 ne reflète **pas** un ordre d'exécution séquentiel. Au sein de M3, deux chaînes s'exécutent en parallèle puis convergent :
> - **Chaîne modèle** : WS-6.1 → WS-6.2 → WS-6.3 → WS-7.1
> - **Chaîne backtest** : WS-8.1 → WS-8.2 → WS-8.3
>
> Les deux convergent vers **WS-7.2** (objectif d'optimisation), qui nécessite à la fois les prédictions (WS-7.1) et le moteur de backtest (WS-8.1/8.2/8.3). Ensuite : WS-7.2 → WS-7.3/WS-7.4 et WS-8.3 → WS-8.4. Le diagramme Mermaid détaillé fait foi pour l'ordre réel.
...
| **Dépendances** | WS-1.1. **Note de placement** : bien que la seule dépendance technique soit WS-1.1 (M1), WS-6.1 est placé en M3 par choix organisationnel — les arcs `T11 --> T61` dans le diagramme Mermaid indiquent une dépendance technique, pas une autorisation de démarrage anticipé. Le séquencement inter-milestones (M1 → M2 → M3) prime. Ce placement permet de stabiliser le pipeline de données (M2) avant de définir le contrat modèle, et de paralléliser WS-6.1 avec WS-8.1 au sein de M3. |
...
| Champ | Valeur |
|---|---|
| **Description** | Module `training/trainer.py`. Orchestrateur d'entraînement par fold : (1) **scaling** (le trainer est le **seul responsable** du scaling : `scaler.fit(X_train)`, `X_train = scaler.transform(X_train)`, `X_val = scaler.transform(X_val)`, `X_test = scaler.transform(X_test)` — l'orchestrateur WS-12.2 ne touche pas au scaling, il le délègue entièrement au trainer), (2) appeler `model.fit(X_train, y_train, X_val, y_val, config, run_dir, meta_train, meta_val, ohlcv)` — le modèle gère en interne l'early stopping (patience configurable). (3) Logger loss train/val, best_epoch. (4) Pas de logique spécifique au modèle dans le trainer — le trainer passe systématiquement `meta_train`, `meta_val` et `ohlcv` ; les modèles supervisés les ignorent, le modèle RL utilise `meta` pour construire son environnement interactif, la SMA baseline utilise `ohlcv` pour ses signaux. (5) Le trainer délègue l'early stopping au modèle via `model.fit()` : les modèles DL implémentent la boucle epoch + patience dans leur `fit()`, XGBoost utilise son API native (`early_stopping_rounds`). Le trainer ne fait **pas** de boucle epoch lui-même — son rôle est l'orchestration du workflow (scale → fit → predict → save), pas la boucle d'entraînement interne. **Séparation des responsabilités predict()** : le trainer est responsable d'appeler `model.predict(X_val)` et `model.predict(X_test)` après le `fit()`. Il retourne les prédictions (`y_hat_val`, `y_hat_test`) à l'orchestrateur (WS-12.2), qui les utilise pour la calibration θ et le backtest. Le modèle lui-même n'a aucune connaissance du contexte d'appel (val vs test). **Note** : le trainer ne génère pas les fichiers `preds_val.csv` / `preds_test.csv` — cette responsabilité incombe à l'orchestrateur (WS-12.2), qui dispose du signal post-calibration θ et de la logique conditionnelle `config.artifacts.save_predictions`. **Note `training.loss` et `training.optimizer`** : ces paramètres sont destinés **exclusivement** aux modèles DL (CNN1D, GRU, LSTM, PatchTST). XGBoost utilise son propre objectif natif (`reg:squarederror` pour MSE) et ignore `training.loss` / `training.optimizer`. L'implémentation du modèle XGBoost **devra logger un warning** si `config.training.loss != "mse"`, car le mapping loss → objectif XGBoost n'est pas automatique dans le MVP. Si un mapping futur est souhaité (mse → `reg:squarederror`, mae → `reg:absoluteerror`, huber → `reg:pseudohubererror`), il devra être géré dans l'implémentation du modèle XGBoost, pas dans le trainer. **Helper DL commun** : pour éviter la duplication de la boucle epoch + early stopping dans les 4 modèles DL (CNN1D, GRU, LSTM, PatchTST), un module `training/dl_train_loop.py` fournira un helper réutilisable `run_dl_training(model, train_loader, val_loader, loss_fn, optimizer, patience, max_epochs) -> TrainResult`. Ce helper encapsule la boucle epoch, le calcul de loss, l'early stopping, et le logging. Chaque modèle DL appelle ce helper dans son `fit()`. **Note** : `dl_train_loop.py` est un composant **post-MVP** (les 4 modèles DL sont post-MVP). Il est documenté ici par anticipation pour guider l'architecture, mais ne sera ni implémenté ni testé tant que le premier modèle DL ne sera pas ajouté. Le livrable MVP de WS-6.3 se limite au `trainer.py` (orchestration fit/predict/scale par fold). |
| **Réf. spec** | §10.3 |
| **Critères d'acceptation** | Le trainer orchestre correctement le workflow complet (scale → fit → predict → save) avec DummyModel sans crash. Prédictions `y_hat_val` et `y_hat_test` retournées. Paramètre `patience` configurable et transmis au modèle. L'early stopping est vérifiable en pratique dès l'ajout d'un modèle DL (le DummyModel valide uniquement le contrat d'appel, pas la convergence). |
| **Dépendances** | WS-6.2, WS-5.1, WS-4.3 (adapter XGBoost, nécessaire si le trainer gère tous les types de modèles). WS-5.2 et WS-5.3 sont des dépendances optionnelles (scalers alternatifs). |
...
| **Dépendances** | WS-7.1, WS-8.1 (exécution des trades), WS-8.2 (cost model pour `net_pnl`), WS-8.3 (equity curve pour `MDD`) |
...
| Champ | Valeur |
|---|---|
| **Position** | Après WS-7.4, avant gate M3. **Note** : en raison des dépendances transitives (WS-7.4 → WS-7.2 → WS-8.1/8.2/8.3), G-Doc est effectivement positionné après tous les WS de M3. Il joue le rôle de pré-vérification structurelle du gate M3 (contrats, registres, attributs) plutôt que de gate de détection précoce intra-milestone. |
| **Objectif** | Vérifier que le contrat plug-in, les registres et les attributs structurels sont cohérents pour les modèles disponibles à M3 (DummyModel) avant de valider le milestone M3. La vérification de complétude des registres (incluant les baselines) est déportée au gate M4. |
| **Critères** | (1) `output_type` déclaré et correct pour DummyModel : `DummyModel.output_type == "regression"`. (2) `execution_mode` par défaut `"standard"` vérifié sur DummyModel. (3) Cohérence registres partielle : `set(MODEL_REGISTRY) ⊆ set(VALID_STRATEGIES)` (le registre modèle est un sous-ensemble des stratégies valides ; la complétude `==` est vérifiée au gate M4 après ajout des baselines). (4) Docstrings conformes au contrat `BaseModel` sur `base.py` et `dummy.py`. (5) Bypass θ fonctionnel pour `output_type == "signal"` vérifié sur données synthétiques. (6) Anti-fuite calibration (G-Leak/Calibration) : θ indépendant de `y_hat_test` (test de perturbation) |
| **Automatisation** | `make gate-doc` → `pytest tests/test_threshold.py tests/test_trainer.py -v` + inspection registres + génération `reports/gate_report_G_Doc.json` |
...
| **Dépendances** | G-Backtest (dépendances techniques : WS-8.3 pour l'equity curve et WS-8.4 pour le journal de trades, toutes deux antérieures à G-Backtest) |
...
| Champ | Valeur |
|---|---|
| **Description** | Module `pipeline/runner.py`. Orchestrer le pipeline complet : (1) charger config, (1b) **vérification cohérence registres** : `assert set(MODEL_REGISTRY) <= set(VALID_STRATEGIES)` — vérifie que tout modèle enregistré correspond à une stratégie valide (sous-ensemble). La complétude `==` n'est exigée qu'au gate M4 (après ajout de toutes les baselines) ; avant M4, seul le sous-ensemble est vérifié, permettant l'ajout progressif des modèles, (2) fixer seed, (3) vérifier l'existence des données brutes (erreur si absentes — l'ingestion est une étape séparée, cf. WS-12.3 `fetch` et WS-12.6 `make fetch-data`) + QA, (4) features, (5) dataset, (5a) **créer `run_dir`** via WS-11.1 (l'arborescence du run est créée ici, avant la phase 2 du logging), (5b) **logging phase 2** : après création du `run_dir` (WS-11.1), ajouter dynamiquement le `FileHandler` vers `run_dir/pipeline.log` (cf. WS-1.1 phase 2 — c'est l'orchestrateur qui est responsable de ce branchement), (6) pour chaque fold : split → **déléguer au trainer** (le trainer effectue scale → fit → predict val → predict test → save ; l'orchestrateur ne fait **pas** le scaling lui-même) → calibrer θ (si `model.output_type == "regression"`, sinon bypass) → **appliquer θ aux prédictions test** (`signals = apply_threshold(y_hat_test, θ)` pour `output_type == "regression"` ; signaux directs pour `output_type == "signal"`) → **écrire `preds_val.csv` et `preds_test.csv`** dans `folds/fold_XX/` (si `config.artifacts.save_predictions = true` ; colonnes `timestamp, y_true, y_hat, signal` ; cf. WS-10.1 pour le format) → backtest (avec `execution_mode = model.execution_mode`, transmis directement de l'attribut de classe) → métriques, (7) agréger inter-fold avec vérification §14.4, (8) écrire artefacts (manifest, metrics, CSVs). L'orchestrateur passe systématiquement `ohlcv` et `meta` au trainer et au modèle via les signatures `BaseModel`. **Important** : le paramètre `ohlcv` passé aux modèles et baselines est le DataFrame OHLCV **complet** couvrant toute la période `[dataset.start, dataset.end[` — et non un sous-ensemble filtré par fold. Cela est nécessaire pour la baseline SMA (E.2.4 : calcul causal sur tout l'historique disponible) et pour le modèle RL (construction de l'environnement avec les prix réels). Les modèles supervisés ignorent ce paramètre. **Gestion des baselines** : si `model.output_type == "signal"`, l'orchestrateur appelle `fit()` (c'est un no-op pour les baselines, mais l'appel valide le contrat BaseModel de bout en bout), **bypass** la calibration θ, et appelle `predict(X_val, meta=meta_val, ohlcv=ohlcv)` puis `predict(X_test, meta=meta_test, ohlcv=ohlcv)` pour obtenir les signaux. Les fichiers `preds_val.csv` et `preds_test.csv` sont générés même pour les baselines et RL (la colonne `y_hat` contient les signaux 0/1). **Format `preds_val.csv`** : identique à `preds_test.csv` (colonnes `timestamp, y_true, y_hat, signal`). **Bypass calibration explicite** : l'orchestrateur **force** `thresholding.method = "none"` et `theta = null` lorsque `model.output_type == "signal"`, indépendamment de la configuration. Ce bypass est loggé au niveau INFO. **Génération conditionnelle des artefacts** : les artefacts sont générés en fonction des flags `config.artifacts.save_*` : si `save_predictions = false`, les fichiers `preds_val.csv` et `preds_test.csv` ne sont pas générés ; si `save_equity_curve = false`, le fichier `equity_curve.csv` n'est pas généré ; si `save_model = false`, les artefacts modèle (poids, paramètres) ne sont pas sauvegardés par fold. Le `manifest.json`, `metrics.json` et `config_snapshot.yaml` sont **toujours** générés (inconditionnels). **Dérivation automatique de `strategy.framework`** : le champ `strategy.framework` du manifest est **dérivé automatiquement** par l'orchestrateur à partir de `strategy.name` via un mapping interne (ex: `dummy` → `none`, `xgboost_reg` → `xgboost`, `cnn1d_reg` → `pytorch`, `gru_reg` → `pytorch`, `lstm_reg` → `pytorch`, `patchtst_reg` → `pytorch`, `rl_ppo` → `pytorch`, `no_trade` → `rule_based`, `buy_hold` → `rule_based`, `sma_rule` → `rule_based`). **`strategy.framework` n'est PAS un champ config YAML** : il n'est pas déclaré dans le modèle Pydantic (incompatible avec `extra="forbid"` défini en WS-1.3). L'orchestrateur le calcule en interne et l'écrit directement dans le manifest (WS-11.2), sans passer par la config. Aucune entrée `strategy.framework` ne doit figurer dans les fichiers YAML. |
...
**Objectif** : valider le pipeline de bout en bout sur données réelles BTCUSDT téléchargées depuis Binance, en conditions grandeur nature (≈73 000 bougies horaires, 2017–2026).
**Réf. spec** : §3, §4, §17
**Milestone** : M6

**Politique fixtures M6** : la règle « pas de fixture pytest ni donnée synthétique — accès réseau réel obligatoire » s'applique aux **tests d'exécution du pipeline** (WS-13.2, WS-13.3) qui valident le run grandeur nature. WS-13.1 (configuration YAML) est un livrable statique (fichier config versionné) dont les tests de validation ne nécessitent ni données ni réseau — l'usage de fixtures conftest standards (`default_config`, `default_yaml_data`) pour comparer les configs et de `tmp_path` pour tester les cas d'erreur (configs corrompues) est autorisé car il ne s'agit pas de fixtures de données OHLCV ni de mocks réseau.
...
│   ├── __init__.py
│   ├── __main__.py                     # WS-12.3 — CLI entry point
│   ├── config.py                       # WS-1.2, WS-1.3
│   ├── data/
│   │   ├── __init__.py
│   │   ├── ingestion.py                # WS-2.1
│   │   ├── qa.py                       # WS-2.2 — QA checks
│   │   ├── missing.py                  # WS-2.3 — Missing candles policy
...
│   ├── models/
│   │   ├── __init__.py                 # Auto-import des modèles (dummy dès M3, baselines ajoutées à M4 — peuple MODEL_REGISTRY)
│   │   ├── base.py                     # WS-6.1
│   │   └── dummy.py                    # WS-6.2
│   ├── training/
│   │   ├── __init__.py
│   │   ├── trainer.py                  # WS-6.3
│   │   └── dl_train_loop.py            # post-MVP — helper DL (boucle epoch + early stopping PyTorch)
│   ├── calibration/
│   │   ├── __init__.py
...
├── tests/
│   ├── test_config.py
│   ├── test_config_validation.py       # WS-1.3
│   ├── test_ingestion.py
│   ├── test_qa.py
│   ├── test_missing.py                 # WS-2.3
│   ├── test_log_returns.py            # WS-3.1
│   ├── test_volatility.py             # WS-3.2
│   ├── test_rsi.py                    # WS-3.3
│   ├── test_ema_ratio.py              # WS-3.4
│   ├── test_volume_features.py        # WS-3.5
│   ├── test_feature_registry.py       # WS-3.6
│   ├── test_feature_pipeline.py       # WS-3.6 (pipeline)
│   ├── test_warmup_validation.py      # WS-3.7
│   ├── test_dataset.py
│   ├── test_splitter.py
│   ├── test_scaler.py
│   ├── test_trainer.py
...
| **Tests** | pytest, un fichier par module source (`test_log_returns.py`, `test_volatility.py`, etc.) sauf regroupement explicite par package pour les modules étroitement couplés (`test_baselines.py` couvre baselines/, `test_artifacts.py` couvre artifacts/), TDD encouragé (RED → GREEN → REFACTOR). Les identifiants de tâche (`#NNN`) sont référencés dans les docstrings des classes/fonctions de test, pas dans le nom des fichiers. Tests unitaires par module + test d'intégration minimal sur données synthétiques vérifiant l'arborescence de sortie, la conformité des JSON et la cohérence des métriques (§17.6). Objectif MVP : couverture > 80% des modules critiques (features, splits, backtest, metrics) comme plancher global. **Note** : les gates de milestone/intra-milestone imposent des seuils spécifiques plus exigeants sur leurs modules cibles (≥ 90% pour G-Features, G-Split, G-Doc et M3, ≥ 95% pour M1 et M4). L'objectif 80% est un plancher pour les modules non couverts par un gate spécifique. **Note I-05** : les noms de fichiers de tests suivent la convention du plan (ex: `test_splitter.py`, `test_scaler.py`, `test_baselines.py`) qui prévaut sur les noms mentionnés dans la spec §17.2 (ex: `test_splits.py`, `test_scaling.py`). |
...
| **ddof** | `ddof=0` (écart-type population) pour les features (§6, WS-3.2 volatilité) ; `ddof=1` (écart-type échantillon) pour l'agrégation inter-fold des métriques (§14, WS-10.3). Les deux choix sont intentionnels et conformes à la spec. |
...
Ce plan utilise des annotations numérotées pour tracer les décisions et clarifications ajoutées au fil des revues. Chaque annotation est identifiée par un préfixe indiquant sa catégorie :

| Préfixe | Signification | Description |
|---|---|---|
| **A-** | **Amendement** | Précision ou ajout fonctionnel au plan d'origine, issu d'une revue de cohérence ou d'un affinement de conception (ex : A-04, A-05, A-07, A-09, A-10). |
| **I-** | **Implementation-defined** | Choix d'implémentation non spécifié dans la spec, documenté pour traçabilité (ex : I-04, I-05, I-06, I-08, I-10). |
| **P-** | **Pratique / convention** | Convention technique transversale (mémoire, précision, tests) applicable à plusieurs workstreams (ex : P-02, P-05). |

**Note sur la numérotation** : les identifiants ne sont pas séquentiels (des numéros manquent dans chaque série). Cela est dû au fait que certaines annotations de revues antérieures ont été intégrées directement dans le texte du plan sans conserver leur identifiant, ou ont été fusionnées avec d'autres. Les identifiants existants sont stables et ne doivent pas être renumérotés, afin de préserver les références croisées dans les tâches et les rapports de revue.
...
flowchart LR
  ...
  subgraph M3["M3 — Training Framework + Backtest Engine"]
    GD((G-Doc)) --> GM3((Gate M3))
  end
  subgraph M4["M4 — Baselines, Métriques & Journal"]
...
| 5 | **G-Doc** | Pré-gate M3 | Après WS-7.4 (effectivement après tous les WS de M3) | Oui (gate M3) | `gate-doc` | `gate_report_G_Doc.json` |
| 6 | **M3** | Milestone | Fin M3 (G-Doc GO) | Oui | `gate-m3` | `gate_report_M3.json` |
