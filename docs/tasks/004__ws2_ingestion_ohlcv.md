...
- [ ] Fichier Parquet généré avec les colonnes canoniques (`timestamp_utc, open, high, low, close, volume, symbol`)
- [ ] Colonnes typées correctement : `timestamp_utc` en datetime64[ns, UTC], prix et volume en float64
- [ ] Tri croissant par `timestamp_utc` vérifié par test
- [ ] SHA-256 stable sur relance (mode idempotent)
- [ ] Pagination correcte : toute la période demandée est couverte
- [ ] Retry avec backoff exponentiel fonctionnel (testé via mock)
- [ ] Cache locale : pas de re-téléchargement si le fichier couvre déjà la période
- [ ] Erreur explicite si le symbole ou l'exchange n'existe pas
- [ ] Convention de bornes `[start, end[` respectée
- [ ] Tests unitaires avec mocks ccxt (pas d'accès réseau dans les tests)
