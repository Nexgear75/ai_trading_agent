...
- [x] Chargement correct de chaque type de CSV avec les bons types de colonnes.
- [x] Colonne `fold` ajoutée automatiquement lors de la concaténation des trades.
- [x] Colonne `costs` calculée (`fees_paid + slippage_paid`).
- [x] Retour `None` si fichier absent (pas d'exception).
- [x] Validation des colonnes obligatoires sur fichier existant (exception si manquantes).
- [x] Tests unitaires avec fixtures CSV synthétiques : fichier valide, fichier absent, colonnes manquantes, concaténation multi-fold.
...
- [x] Branche `task/075-wsd1-data-loader-csv` créée depuis `Max6000i1`.
- [x] Tests RED écrits avant implémentation.
- [x] **Commit RED** : `[WS-D-1] #075 RED: tests data loader chargement CSV` (fichiers de tests uniquement).
