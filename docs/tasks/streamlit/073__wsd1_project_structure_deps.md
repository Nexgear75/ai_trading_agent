...
- [ ] Arborescence `scripts/dashboard/` créée avec tous les fichiers (cf. §10.2).
- [ ] `scripts/dashboard/pages/` contient les 4 fichiers de pages.
- [ ] `.streamlit/config.toml` conforme à §10.4.
- [ ] `pip install -r requirements-dashboard.txt` passe sans conflit avec les dépendances existantes.
...
- [ ] Branche `task/073-wsd1-project-structure-deps` créée depuis `Max6000i1`.
- [ ] Tests RED écrits avant implémentation.
- [ ] **Commit RED** : `[WS-D-1] #073 RED: tests structure projet et dépendances dashboard` (fichiers de tests uniquement).
- [ ] Tests GREEN passants et reproductibles.
- [ ] Critères d'acceptation tous satisfaits.
- [ ] `ruff check ai_trading/ tests/` passe sans erreur.
- [ ] Fichier de tâche mis à jour (statut DONE, critères cochés).
- [ ] **Commit GREEN** : `[WS-D-1] #073 GREEN: structure projet et dépendances dashboard`.
