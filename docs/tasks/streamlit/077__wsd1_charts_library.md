...
- [ ] Chaque fonction génère un `go.Figure` Plotly valide (pas de `None`).
- [ ] Couleurs conformes à §9.2 (palette centralisée).
- [ ] `chart_equity_curve()` : fold boundaries détectées par changement colonne `fold`, drawdown ombré, zones in_trade.
- [ ] `chart_equity_overlay()` : normalisation equity à 1.0, légende cliquable.
- [ ] `chart_radar()` : 5 axes, normalisation min-max sur les runs fournis.
- [ ] `chart_scatter_predictions()` : coloration Go/No-Go, message informatif si `method == "none"`.
- [ ] `chart_fold_equity()` : marqueurs ▲ vert (entrée) et ▼ rouge (sortie).
- [ ] Tests unitaires avec données synthétiques (pas de test visuel) : vérification du type `go.Figure`, nombre de traces, axes, annotations.
...
- [ ] Branche `task/077-wsd1-charts-library` créée depuis `Max6000i1`.
- [ ] Tests RED écrits avant implémentation.
- [ ] **Commit RED** : `[WS-D-1] #077 RED: tests bibliothèque graphiques Plotly` (fichiers de tests uniquement).
- [ ] Tests GREEN passants et reproductibles.
- [ ] Critères d'acceptation tous satisfaits.
- [ ] `ruff check ai_trading/ tests/` passe sans erreur.
- [ ] Fichier de tâche mis à jour (statut DONE, critères cochés).
- [ ] **Commit GREEN** : `[WS-D-1] #077 GREEN: bibliothèque graphiques Plotly`.
