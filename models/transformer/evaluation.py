"""
Évaluation spécifique au modèle Transformer.

Thin wrapper autour de utils.evaluation : gère le chargement du modèle
Transformer et de ses scalers, puis délègue les métriques et graphiques au
...
def _get_checkpoint_paths(timeframe: str):
    ...
    checkpoint_dir = f"models/transformer/checkpoints/{timeframe}"
    return {
        "dir": checkpoint_dir,
        "model": os.path.join(checkpoint_dir, "best_model.pth"),
        "scalers": os.path.join(checkpoint_dir, "scalers.joblib"),
        "results": f"models/transformer/results/{timeframe}",
    }


def load_model(model_path: str, device: torch.device, window_size: int = None) -> tuple[Transformer, dict]:
    """Charge le modèle Transformer et son historique d'entraînement.
...
def evaluate(
    ...
    """Évalue le modèle Transformer et génère les graphiques.
    ...
    print(f"\n{'=' * 60}")
    print(f"  ÉVALUATION TRANSFORMER")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Évaluation Transformer")
