...
class TestDataPreparator:
    """Tests pour prepare_data (réexport du CNN)."""
...
class TestConfig:
    ...
    def test_fallback_config(self):
        """Un timeframe sans config explicite retourne un fallback valide."""
        cfg = get_patchtst_config("4h")
        assert "patch_len" in cfg
        assert "stride" in cfg
        assert cfg["d_model"] % cfg["n_heads"] == 0
        # Vérifier que num_patches >= 2
        window_size = get_timeframe_config("4h")["window_size"]
        num_patches = (window_size - cfg["patch_len"]) // cfg["stride"] + 1
